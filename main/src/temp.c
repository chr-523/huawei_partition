...
void Graph::read_graph_data(const Module& gra_data,  Name_type& prefix, Name_type& mod_index) {
    ...
    for( auto& ins_data: gra_data.get_internal_instance() ){
        ...
        for(auto& e_name : ins_data.get_connect_edge() ){
            ...
            if ( e_data != edge_map.end() ){
                ...
                if ( e_data -> second -> get_type() == INPUT ){
                    ...
                    edge_pin_info.push_back({mod_index + "_" + e_name, *e_data->second});
                }
        ...
        if(!is_sub_module_empty){
            for (const auto& pair : this -> pin_temp ){
                ...
                while(!pair_temp.empty()){
                    auto edge_data = mod_pin_map.find(std::get<0>(pair_temp.front()));
                    pair_temp.pop_back();
                    if(edge_data != mod_pin_map.end() ){
                        auto v2m = edge_map.find(edge_data -> second.first);
                        mod_to_ins[pair.first] = {v2m->second->get_name(),std::get<1>(pair_temp.front())};
                        // pinedge?s[pair.first] = {v2m->second->get_name(),std::get<1>(pair_temp.front())};
                        int a = 1;
                    }
                }
            }
        ...
        if( !(edge_pin_info.empty()) ){
            add_edge_pin_temp(all_name, edge_pin_info);
        }
    ...
    if(!is_sub_module_empty){
        ...
        for (const auto& pair : mod_to_ins) {
            ...
            Name_type v_e_name = std::get<0>(pair.second);
            std::cout << v_i_name<<" - - - "<<v_e_name << std::endl;
            auto edge_data = edge_map.find(v_e_name);
            Edge_type edge_dire = std::get<1>(pair.second);
            Direction dire_f;
            if(edge_dire == OUTPUT){ dire_f = edge_to_v;}
            else{ // input:被处理顶点被输入，需要处理另一个v 指向边的的节点
                dire_f = v_to_edge; 
            } 
            ...
            // }
                ...
                for(size_t ins_index = 0; ins_index < adj_array.size(); ++ins_index) {
                    ...
                    if( adj_array[ins_index].back() == '*' ){
                        if( array_direction[ins_index] == dire_f ){ 
                            //同向意味着指向 现在处理的ins 不处理
                            std::vector< std::pair< Name_type, weight_type > > adj_;
                            if (dire_f == edge_to_v) {
                                auto it = pin_mod_map.find(v_e_name+"_i");
                                int v = 1;
                            }
                            else{ // dire_f == v_to_edge
                                auto it = pin_mod_map.find(v_e_name+"_o");
                                int c = 1;
                            }
                            
                            int a = 1;//吧需要的ins 里面的加入a2u1 的minus或者plus
                            add_adj_plus(v_i_name, adj_);
