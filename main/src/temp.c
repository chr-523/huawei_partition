    ...
    if(!is_sub_module_empty){
        for (const auto& i_p : this -> ins_pin_edge_temp ){
            ...
            while(!i_p_temp.empty()){
                ...
                auto out_edge_ = pin_outedge_map.find(pin_name);
                if (out_edge_ == pin_outedge_map.end()){
                    // out_edge_ =;
                    int a = 1;
                }
                auto out_edge = out_edge_ -> second;
                ...
                if(!is_IO){ // not io -> mod-mod/mod-ins
                    ...
                    // clear后加回temp hrere
                    add_pin_outedge_temp(pin_name,out_edge);
                    Name_type subfix = "_i";
                    if(out_edge.second == OUTPUT){
                        subfix = "_o";
                    }
                    Name_type check_pin_name = o_e_name + subfix;
                    Name_type other_pin_name = outedge_pin_map[check_pin_name];
                    add_outedge_pin_temp(check_pin_name,other_pin_name);
                    int b = 1;
                }
